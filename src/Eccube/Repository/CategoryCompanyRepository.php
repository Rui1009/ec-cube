...
/**
 * CategoryCompanyRepository
 *
 * This class was generated by the Doctrine ORM. Add your own custom
 * repository methods below.
 */
class CategoryCompanyRepository extends AbstractRepository
{
    /**
     * @var EccubeConfig
     */
    ...
    /**
     * CategoryCompanyRepository constructor.
     */
     ...
     *
     * @param CategoryCompany|null $Parent 指定の親カテゴリ
     * @param bool $flat trueの場合, 階層化されたカテゴリを一つの配列にまとめる
     *
     * @return CategoryCompany[] カテゴリの配列
     */
    public function getList(CategoryCompany $Parent = null, $flat = false)
    {
     ...
     * カテゴリを保存する.
     *
     * @param  CategoryCompany $CategoryCompany カテゴリ
     */
    public function save($CategoryCompany)
    {
        if (!$CategoryCompany->getId()) {
            $Parent = $CategoryCompany->getParent();
            if ($Parent) {
            ...
            $CategoryCompany->setSortNo($sortNo + 1);
        ...
        $em->persist($CategoryCompany);
        $em->flush($CategoryCompany);
    }

    /**
     * カテゴリを削除する.
     *
     * @param  CategoryCompany $CategoryCompany 削除対象のカテゴリ
     *
     * @throws ForeignKeyConstraintViolationException 外部キー制約違反の場合
     * @throws DriverException SQLiteの場合, 外部キー制約違反が発生すると, DriverExceptionをthrowします.
     */
    public function delete($CategoryCompany)
    {
        $this
            ->createQueryBuilder('c')
            ->update()
            ->set('c.sort_no', 'c.sort_no - 1')
            ->where('c.sort_no > :sort_no')
            ->setParameter('sort_no', $CategoryCompany->getSortNo())
            ->getQuery()
            ->execute();

        $em = $this->getEntityManager();
        $em->remove($CategoryCompany);
        $em->flush($CategoryCompany);
    }
}
